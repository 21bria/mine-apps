...
import json
from ....models.sample_grade_control_model import GradeExpectationsRoa
...
@login_required      
def grade_expectations_roa_page(request):
    ...
    return render(request, 'admin-mgoqa/report-gc/list-grade-expectations-roa.html',context)

@login_required
def grade_expect_roa_page(request):
    allowed_groups = ['superadmin','admin-mgoqa','superintendent-mgoqa','manager-mgoqa','user-mgoqa','data-control']
    if not request.user.groups.filter(name__in=allowed_groups).exists():
        # Jika tidak memiliki izin, arahkan ke halaman error
        context = {
    ...
    return render(request, 'admin-mgoqa/report-gc/grade-expect-chart-roa.html',context)

@login_required
def geos_expect_roa_page(request):
    permissions = get_dynamic_permissions(request.user)
    context = {
            'permissions' : permissions,
    } 
    return render(request, 'admin-mgoqa/report-gc/geos-expect-chart-roa.html',context)

class GradeExpectations_roa(View):
    ...
    def _datatables(self, request):
        ...
        data = GradeExpectationsRoa.objects.all()
...
def scatterGradeExRoa(request):
    ...
    fig.update_layout(
            ...
            title='Ni expectation vs actual (ROA)',
            hovermode='closest',
...
def lineSampleExRoa(request):
    ...
    query = """
          SELECT
                ...
                LEFT JOIN assay_roas as t4 ON t4.sample_id=t3.sample_number
    ...
    try:
        ...
        fig.update_layout(
            ...
            title   ='Data Expectation by Samples (ROA)' ,
...
def lineDateExRoa(request):
    ...
    try:
        ...
        fig.update_layout(
            title   ='Data Expectation by Date (ROA)',
            xaxis=dict(
                    title='Date',
                    title_font=dict(size=12),
                    tickformat='%Y-%m-%d',  # Pastikan format tick tetap dalam bentuk string
                    # tickangle=-45,  # Rotasi otomatis (contoh: -45 derajat)
                ),
            yaxis=dict(
                title='Expectation',
                titlefont=dict(color='blue'),
...
def lineGeosExRoa(request):
    ...
    try:
        ...
        fig.update_layout(
            title   ='Data Expectation by GC (ROA)',
            xaxis_title ='Grade Control',
            xaxis_title_font=dict(size=12),
            yaxis=dict(
                title='Expectation',
                titlefont=dict(color='blue'),
...
# Group Mine Geos  
@login_required
def scatterMineGeosExRoa(request):
    startDate = request.GET.get('startDate')
    endDate   = request.GET.get('endDate')
    material  = request.GET.get('material')
    ...
    fig.update_layout(
            ...
            title=f'Ni (ROA) analys -  {gradeControl}',
            hovermode='closest',
            plot_bgcolor='rgba(201,201,201,0.08)',
            margin=dict(l=10, r=10, t=70, b=20), 
            legend= dict(
...
def lineMineGeosExRoa(request):
    ...
    try:
        ...
        fig.update_layout(
            # title  ='Data Expectation by Date (ROA)',
            title=f'Data by Date (ROA) -  {gradeControl}',
