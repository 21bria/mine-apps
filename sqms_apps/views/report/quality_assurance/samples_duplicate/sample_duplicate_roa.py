...
@login_required
def sampleDuplicateWetRoa(request):
    ...
    return render(request, 'admin-mgoqa/report-qa/wet-duplicated-roa.html',context)

@login_required
def sampleDuplicateRoa(request):
    allowed_groups = ['superadmin','admin-mgoqa','superintendent-mgoqa','manager-mgoqa','user-mgoqa','data-control']
    if not request.user.groups.filter(name__in=allowed_groups).exists():
        # Jika tidak memiliki izin, arahkan ke halaman error
        context = {
    ...
    # form = DateFilterForm(request.GET or None)
    return render(request, 'admin-mgoqa/report-qa/list-samples-duplicated-roa.html',context)

# plotly | Graphing Libraries
@login_required
def chart_wet_year_roa(request):
    ...
    colors      = ['rgba(0, 150, 136, 0.8)', 'rgba(0, 150, 136, 0.3)']
    ...
    fig.update_layout(
        title=f'Wet duplicated roa of year ({filter_year})',
...
@login_required
def chart_wet_month_roa(request):
  # Mendapatkan nilai filter dari request
    ...
    # Menambahkan garis untuk Centre Line
    top_labels = ['Acceptable Sample', 'Error Sample',]
    colors     = ['rgba(255, 167, 55, 0.8)', 'rgba(255, 167, 55, 0.3)']
    ...
    fig.update_layout(
       title=f'Wet duplicated roa of month, {filter_month}-{filter_year}',
        title_font=dict(size=19),
        margin=dict(l=25, r=20, t=90, b=80),
        title_x=0.5,
        barmode='stack',
...
@login_required
def chart_wet_week_roa(request):
  # Mendapatkan nilai filter dari request
    ...
    # Menambahkan garis untuk Centre Line
    top_labels = ['Acceptable Sample', 'Error Sample',]
    colors     = ['rgba(216, 27, 96, 0.8)', 'rgba(216, 27, 96, 0.3)']
    ...
    fig.update_layout(
        title=f'Wet duplicated roa, {startDate} to {endDate}',
        title_font=dict(size=19),
        margin=dict(l=25, r=20, t=90, b=80),
        title_x=0.5,
        barmode='stack',
...
#List Data Tables:
class SamplesDuplicatedRoa(View):
