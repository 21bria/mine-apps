...
@login_required
def scatter_plot_dup_mral(request):
    ...
    return render(request, 'admin-mgoqa/report-qa/scatter-duplicated-mral.html',context)

@login_required
def scatter_ploty_ni(request):
...
@login_required
def scatter_ploty_sio2(request):
    start_date = request.GET.get('start_date')
    end_date = request.GET.get('end_date')
